...
public class ex38_base {
    public static void main(String[] args){
        numberList run = new numberList();
        List<String> list = run.makeList();
        ArrayList<Integer> numbers = run.convertList(list);
        numbers = run.filterEvenNumbers(numbers);
        run.printList(numbers);
    }
}
